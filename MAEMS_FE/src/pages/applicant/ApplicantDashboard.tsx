...
const statusConfig: Record<ApplicationStatus, { label: string; color: string; antColor: string; Icon: React.ElementType }> = {
  draft:        { label: "Bản nháp",       color: "text-gray-500",  antColor: "default",    Icon: CircleDot      },
  submitted:    { label: "Đã nộp",         color: "text-blue-500",  antColor: "blue",       Icon: Clock          },
...
function overallStatus(apps: Application[]): ApplicationStatus | null {
...
const DEADLINES = [
  { label: "Hạn nộp hồ sơ xét học bạ",        date: "2026-04-15", urgent: true  },
  { label: "Hạn nộp hồ sơ xét thi THPT",       date: "2026-07-20", urgent: false },
  { label: "Công bố kết quả xét tuyển đợt 1",  date: "2026-08-05", urgent: false },
  { label: "Xác nhận nhập học",                 date: "2026-08-20", urgent: false },
];
...
export function ApplicantDashboard() {
  ...
  const displayName  = applicant?.fullName ?? user?.username ?? user?.email ?? "Thí sinh";

  // stats
  const countByStatus = (s: ApplicationStatus) => apps.filter((a) => a.status === s).length;
  const stats = [
    { label: "Hồ sơ đã nộp",    value: apps.length,               color: "text-orange-500", bg: "bg-orange-50",  border: "border-orange-100" },
    { label: "Đang xét duyệt",  value: countByStatus("under_review"), color: "text-blue-600",   bg: "bg-blue-50",    border: "border-blue-100"   },
    { label: "Đã chấp nhận",    value: countByStatus("approved"),   color: "text-green-600",  bg: "bg-green-50",   border: "border-green-100"  },
    { label: "Chờ bổ sung",     value: countByStatus("draft"),      color: "text-orange-500", bg: "bg-orange-50",  border: "border-orange-100" },
  ];
  ...
  const quickActions = [
    { label: "Đăng ký ngành mới",    icon: FilePlus,                path: "/applicant/submit-application", color: "bg-orange-500 hover:bg-orange-600" },
    { label: "Nộp / bổ sung tài liệu", icon: Paperclip,             path: "/applicant/applications",       color: "bg-orange-600 hover:bg-orange-700" },
    { label: "Xem đơn đăng ký",      icon: FileText,                path: "/applicant/applications",       color: "bg-blue-500 hover:bg-blue-600"     },
    { label: "Cập nhật hồ sơ",       icon: UserRound,               path: "/applicant/profile",            color: "bg-teal-500 hover:bg-teal-600"     },
    { label: "Tìm hiểu ngành học",   icon: BookOpen,                path: "/tuyen-sinh",                   color: "bg-purple-500 hover:bg-purple-600" },
    { label: "Hỏi đáp tuyển sinh",   icon: MessageCircleQuestion,   path: "/tuyen-sinh",                   color: "bg-pink-500 hover:bg-pink-600"     },
  ];
        ...
        <div className="flex flex-col gap-5">
          ...
          <div className="rounded-2xl bg-gradient-to-r from-orange-500 to-orange-400 p-6 flex flex-col sm:flex-row items-start sm:items-center gap-5 shadow-md">
            ...
            <div className="flex-1 min-w-0">
              ...
              {applicant && (
                <Text className="!text-orange-200 !text-xs">
                  {applicant.contactEmail} &nbsp;·&nbsp; {applicant.contactPhone}
                </Text>
              )}
            ...
            <div className="w-full sm:w-52 shrink-0">
              <div className="flex items-center justify-between mb-1.5">
                <Text className="!text-orange-200 !text-xs">Mức độ hoàn thiện hồ sơ</Text>
                <Text className="!text-white !text-xs !font-bold">{completion}%</Text>
              </div>
              <Progress
                percent={completion}
                showInfo={false}
          ...
          <Row gutter={[16, 16]}>
            {stats.map((s) => (
              <Col xs={12} md={6} key={s.label}>
                <Card
                  className={`rounded-2xl border ${s.border} ${s.bg} shadow-sm hover:shadow-md transition-shadow cursor-default`}
                  styles={{ body: { padding: "16px 20px" } }}
                >
                  <Text className="text-xs text-gray-500 uppercase tracking-wide block">
                    {s.label}
                  </Text>
                  <div className={`text-4xl font-extrabold mt-1 ${s.color}`}>
                    {s.value}
                  </div>
                </Card>
              </Col>
          ...
          <Row gutter={[16, 16]}>
              ...
              >
                ...
                {recentApps.length === 0 ? (
                  <div className="flex flex-col items-center gap-2 py-8">
                    ...
                    <Text className="text-gray-400 text-sm">Bạn chưa có đơn đăng ký nào.</Text>
                    <Button
                      type="primary"
                      size="small"
                        ...
                        >
                          ...
                          <div className="flex-1 min-w-0">
                            ...
                            <Text className="text-[11px] text-gray-400">
                              {app.admissionTypeName} &nbsp;·&nbsp; {app.campusName}
                              {app.lastUpdated && <> &nbsp;·&nbsp; {formatDate(app.lastUpdated)}</>}
                            </Text>
                          </div>
            ...
            <Col xs={24} lg={10}>
              <Card
                ...
                styles={{ body: { padding: "20px 24px" } }}
              >
                <Title level={5} className="!mb-4 !text-gray-700 !font-semibold">
                  Thao tác nhanh
                </Title>
                ...
                return (
                  ...
                  >
                    <div className={`w-8 h-8 rounded-lg flex items-center justify-center shrink-0 ${
                      passed ? "bg-gray-200" : soon || d.urgent ? "bg-red-100" : "bg-orange-50"
                    }`}>
                      {passed ? (
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <Text className={`text-xs ${passed ? "text-gray-400" : soon || d.urgent ? "text-red-500 font-semibold" : "text-gray-400"}`}>
                        {passed
                          ? "Đã qua hạn"
                          : days === 0
                          ? "Hôm nay là hạn chót!"
                          : `Còn ${days} ngày — ${formatDate(d.date)}`}
                      </Text>
                    </div>
                  </div>
                );
          ...
          {!applicant && (
            <div className="rounded-2xl bg-amber-50 border border-amber-200 p-4 flex items-start gap-3">
              <CircleAlert size={20} className="text-amber-500 shrink-0 mt-0.5" />
