...
export function SubmitDGNL() {
  ...
  useEffect(() => {
    Promise.all([
      ...
      setAdmissionTypes(
        (admTypesData ?? []).filter(
          (t) => t.type === "PT2" || t.admissionTypeName?.toLowerCase().includes("năng lực")
        )
      );
      setLoading(false);
    });
      ...
      <div className="max-w-2xl mx-auto">
        ...
        <div className="flex items-center gap-3 mb-8">
          <div className="w-10 h-10 rounded-xl bg-orange-50 border border-orange-200 flex items-center justify-center">
            <FileCheck2 size={20} className="text-orange-500" />
          </div>
          <div>
            <Text className="text-xs text-gray-400 uppercase tracking-wider">
              PT2 — Phương thức xét tuyển
            </Text>
            ...
            <div className="rounded-2xl bg-white border border-gray-100 shadow-sm p-6 md:p-8">
              <Form form={form} layout="vertical" onFinish={handleSubmit} requiredMark={false}>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-x-4">
                  ...
                  >
                    <Select
                      placeholder="Chọn phương thức ĐGNL"
                      size="large"
                      className="w-full"
